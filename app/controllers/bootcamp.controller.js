...
// buscar por bootcamp incluyendo los usuarios
exports.buscarBootcampUsuarios = async (req, res) => {
  ...
  try {
    ...
    if (bootcamp) {
      ...
      res.status(404).send({
        message: `No se encontró el Bootcamp con id=${id}.`
      });
    }
  } catch (err) {
    res.status(500).send({
      message: err.message || "Ocurrió un error al obtener el Bootcamp."
    });
  }
};